...
int perturbations_vector_init(
  ...
  if (_scalars_) {
    ...
    class_define_index(ppv->index_pt_theta_cdm, pba->has_cdm && ((ppt->gauge == newtonian) || (pba->w_dm_0 != 0.0)), index_pt, 1); /* Hanyu cdm velocity */
...
int perturbations_derivs(double tau,
  ...
  if (_scalars_) {
    ...
    /** - ---> cdm */
    /* Hanyu CDM with variable equation of state */
    if (pba->has_cdm == _TRUE_) {
      // Get CDM equation of state w and its derivative
      double w_cdm;
      double w_prime_cdm;
  
      // Calculate w_cdm at this scale factor
      class_call(background_w_cdm(pba, a, &w_cdm),
                ppt->error_message,
                ppt->error_message);
  
      // Calculate w'_cdm (derivative with respect to conformal time)
      double dw_over_da;
      if (a >= pba->a_nz) {
        dw_over_da = pba->w_dm_0 / (1.0 - pba->a_nz); // Derivative of w_dm(a)
      } else {
        dw_over_da = 0.0;
      }
      w_prime_cdm = dw_over_da * a * a_prime_over_a;
  
      /** - ----> newtonian gauge: cdm density and velocity with equation of state */
      if (ppt->gauge == newtonian) {
        // Modified density equation with w_cdm (simplified with cs2_cdm = 0)
        dy[pv->index_pt_delta_cdm] = -(1.0 + w_cdm) * (y[pv->index_pt_theta_cdm] + metric_continuity) 
                                  + 3.0 * a_prime_over_a * w_cdm * (y[pv->index_pt_delta_cdm] + 3.0 * a_prime_over_a * (1.0 + w_cdm) * y[pv->index_pt_theta_cdm] / k2)
                                  - 3.0 * a_prime_over_a * w_prime_cdm * y[pv->index_pt_theta_cdm] / k2;
    
        // Modified velocity equation with w_cdm (simplified with cs2_cdm = 0)
        dy[pv->index_pt_theta_cdm] = -a_prime_over_a * y[pv->index_pt_theta_cdm] + metric_euler;
      }
  
      /** - ----> synchronous gauge: cdm density and velocity */
      if (ppt->gauge == synchronous) {
        if (pba->w_dm_0 != 0.0) {
          // The CDM density evolution (simplified with cs2_cdm = 0)
          dy[pv->index_pt_delta_cdm] = -(1.0 + w_cdm) * (y[pv->index_pt_theta_cdm] + metric_continuity)
                                    + 3.0 * a_prime_over_a * w_cdm * (y[pv->index_pt_delta_cdm] + 3.0 * a_prime_over_a * (1.0 + w_cdm) * y[pv->index_pt_theta_cdm] / k2)
                                    - 3.0 * a_prime_over_a * w_prime_cdm * y[pv->index_pt_theta_cdm] / k2;
      
          // The CDM velocity evolution (simplified with cs2_cdm = 0)
          dy[pv->index_pt_theta_cdm] = -a_prime_over_a * y[pv->index_pt_theta_cdm];
        } else {
          // If w_cdm is zero, use the original equation
