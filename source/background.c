...
/*   if (pba->has_cdm == _TRUE_) {
  ...
  /* cdm - with variable equation of state Hanyu*/
  if (pba->has_cdm == _TRUE_) {
    /* Define parameters from background structure */
    double a_nz = pba->a_nz;
    double w_dm_0 = pba->w_dm_0;
    
    /* Calculate w_dm at this scale factor */
    double w_dm;
    if (a >= a_nz) {
        w_dm = w_dm_0 * (a - a_nz) / (1.0 - a_nz);
    } else {
        w_dm = 0.0;
    }
    pvecback[pba->index_bg_w_cdm] = w_dm;
    
    /* Calculate CDM density using the provided expressions */
    double rho_dm;
    if (a < a_nz) {
        double exponent = 3.0 * w_dm_0 / (1.0 - a_nz) * (1.0 - a_nz + a_nz * log(a_nz));
        rho_dm = pba->Omega0_cdm * pow(pba->H0, 2) * pow(a, -3) * exp(exponent);
    } else {
        double exponent = 3.0 * w_dm_0 / (1.0 - a_nz) * (1.0 - a + a_nz * log(a));
        rho_dm = pba->Omega0_cdm * pow(pba->H0, 2) * pow(a, -3) * exp(exponent);
    }
    pvecback[pba->index_bg_rho_cdm] = rho_dm; 
  
    /* Add CDM contribution to total density */
    rho_tot += pvecback[pba->index_bg_rho_cdm];
  
    /* Calculate and add CDM pressure: p = w * rho */
    pvecback[pba->index_bg_p_cdm] = w_dm * pvecback[pba->index_bg_rho_cdm];
    p_tot += pvecback[pba->index_bg_p_cdm];
  
    /* Compute dp_dloga */
    double dw_over_da;
    if (a >= a_nz) {
        dw_over_da = w_dm_0 / (1.0 - a_nz); /* Derivative of w_dm(a) */
    } else {
        dw_over_da = 0.0;
    }
    dp_dloga += (a * dw_over_da - 3.0 * (1.0 + w_dm) * w_dm) * pvecback[pba->index_bg_rho_cdm];
...
int background_indices(
  ...
  class_define_index(pba->index_bg_p_cdm,pba->has_cdm,index_bg,1); /* Hanyu */
  class_define_index(pba->index_bg_w_cdm,pba->has_cdm,index_bg,1);
...
int background_output_titles(
  ...
  class_store_columntitle(titles,"(.)p_cdm",pba->has_cdm); /* Hanyu */
  class_store_columntitle(titles,"w_cdm",pba->has_cdm); /* Hanyu */
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++) {
    ...
    class_store_double(dataptr,pvecback[pba->index_bg_p_cdm],pba->has_cdm,storeidx); /* Hanyu */
    class_store_double(dataptr,pvecback[pba->index_bg_w_cdm],pba->has_cdm,storeidx); /* Hanyu */
...
/**
 * Hanyu Equation of state for CDM with variable w
 *
 * @param pba            Input: pointer to background structure
 * @param a              Input: scale factor
 * @param w_cdm          Output: CDM equation of state at scale factor a
 * @return the error status
 */
int background_w_cdm(
    struct background * pba,
    double a,
    double * w_cdm
  ) {
